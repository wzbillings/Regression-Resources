...
for us, since that's not the focus of this.

Because we know what probability distribution this statistic follows,
we can find values where our statistic will occur with a certain probability
range. We typically set $\alpha = 0.05$, and this is the same idea -- we want
to find values of the $t$-statistic that should occur $5\%$ of the time or
less often.
